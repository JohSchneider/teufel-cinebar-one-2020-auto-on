...
| `firmware_37_usben-forced.bin` | 36 | **Experimental USB-MSC test build, layer 2.** fw_36 + a 32-byte shim at `0x0801E880` that gets called instead of the original `bl 0x080030B0` at `0x08003AD0` (`main()`'s USB stack init call site). The shim sets `RCC_APB1ENR \|= (1 << 23)` (USBEN), then calls the original USB init, then returns. Goal: prove the hypothesis "MSC works if we just enable the clock" — closes out whether the entire MSC chain is missing only the clock-enable line or whether other prerequisites (USB clock source, BCDR.DPPU, descriptor callbacks) are also missing. For the full test, T211 must also be externally pulled HIGH. Not yet bench-flashed. |
