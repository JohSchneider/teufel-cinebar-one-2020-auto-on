...
| **`firmware_22_wake-on-spdif.bin`** | 01 | ★ **Productive** | Goal #1 + Goal #2 complete: auto-on + wake-on-SPDIF + auto-suspend on silence ~15 min. **Use this unless you need an experimental feature.** |
| `firmware_23_music-mode-default.bin` | 22 | ✓ Optional | + 24-byte Shim 3 at `0x0801E880`: after every `transition_state(2)` (wake), calls `set_audio_mode(0)` to force Music. Useful only if the bar auto-switches mode on detected audio format. |
| `firmware_32_preloaded-music-vol35.bin` | 22 | ✓ Optional | fw_22 with vEEPROM page appended: `vol=35, bass=8, modeExtend=1, mode=Music`. Bar boots into those settings on first flash. |
...
3. **Rail-up needs PA2, PB7, AND PC15 all HIGH.** Missing any of them keeps the rail off. PF0 is also required for PA4 to track SPDIF when the rail is held up by an explicit standby NOP.
