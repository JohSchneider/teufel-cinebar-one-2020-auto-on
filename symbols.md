...
| `0x0800EE9C`  | ★★★  | `cec_peripheral_init`             | **CEC peripheral init** (confirmed 2026-06-09, task #65). Enables GPIOA clock, configures PA5 as AF1, sets CECEN (`RCC_APB1ENR` bit 30) via the `lsls r0, r4, #18`/r4=`0x40021000` trick (= `0x40000000`), toggles APB1RSTR bit 30 to reset CEC, initializes the CEC handle struct at RAM `0x200026F8` (with `[+0]=0x40007800`=CEC base, `[+8]=8`, `[+12]=16`, `[+36]=15`, `[+40]=state+0x88` buffer), calls HAL CEC state-machine helpers `0x0800D064` + `0x0800D1EC`, enables NVIC IRQ 30 (CEC_CAN) at priority 1. Called by `0x0800F132` (= second instruction of `0x0800F130`). |
| `0x0800F00C`  | ★★★  | `i2c2_init`                       | I²C2 peripheral init for the service-mode EEPROM bus. Enables GPIOB clock + I²C2EN. Called by `0x0800F138` (second instruction sequence in `0x0800F130`). |
| `0x0800F130`  | ★★★  | `service_peripheral_init`         | Calls `cec_peripheral_init` then `i2c2_init`. Sets up both peripherals the service mode needs. Called via `0x0800F154` (wrapper) from `0x0800E980` (the "main service work" branch of the service thread's LOW path). |
| `0x0800E8A0`  | ★★    | `service_dispatch`                | Was previously called `service_mode_main`. Inside, when `r0==5`, runs a CEC operation on the handle at `0x200026F8` using helpers `0x800D064`/`0x800D1EC`/`0x800D284`/`0x800D0E0`. **`r0` is the operation index, not a magic constant** — `state[0]==5` in the calling thread's struct is what drives this. The function only implements the `r0==5` branch explicitly; other values fall through to early-return. Likely additional dispatch values are EEPROM-supplied. |
| `0x0800D064`  | ★    | `HAL_CEC_helper_1` (likely HAL_CEC_DeInit or similar) | CEC HAL state-machine helper. Writes `[handle+32+21] = 0x24` (state byte = 36 = some HAL state). Called by both `cec_peripheral_init` and `service_dispatch(r0=5)`. |
| `0x0800D1EC`  | ★    | `HAL_CEC_helper_2`                | Another CEC HAL helper. Checks `[handle+40]`, validates `[handle+32+21]` state byte. Probably HAL_CEC_Init or HAL_CEC_Start. |
| `0x0800D284`  | ★    | `HAL_CEC_helper_3`                | More CEC state machine work. Checks `[handle+32+21]==32`, sets to 33 (= 0x21). Looks like a transmit/receive entry point. |
| `0x0800D0E0`  | ★    | `HAL_CEC_helper_4` (likely HAL_CEC_GetState) | Reads `[handle+56]` — returns 0 if no error, error code otherwise. Used by `service_dispatch` to test bit 12 of return value for "operation failed". |
...
| PA1   | Input VeryHigh no-pull | **★ Chassis SUB PAIRING button** (verified live 2026-06-09 via GPIO scan). Idle HIGH (external pull-up); pressing the button pulls it LOW. Configured by `HAL_GPIO_Init` at `0x0800B456` (app) and also by the bootloader at boot. **This is the bootloader's MSC firmware-update entry gesture** — hold the sub-pairing button while powering on the bar → bootloader sees PA1 LOW → skips app-validity check → enters USB MSC mode. (Earlier mislabel as "IR receiver" disproven by GPIO scan — IR signal doesn't toggle PA1.) |
| PB1   | (no init found in app) | **★ IR receiver output** (verified live 2026-06-09 via GPIO scan — toggles only during IR remote activity). Probably feeds into the daughter board's IR decode logic; the STM32 doesn't appear to read PB1 directly. IR-decoded events reach the STM32 via PA7/PA15 (daughter-board signals) instead. |
